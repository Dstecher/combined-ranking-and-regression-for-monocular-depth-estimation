...
# Original code can be found at https://github.com/ialhashim/DenseDepth/blob/master/callbacks.py
...
def get_nyu_callbacks(model, basemodel, train_generator, test_generator, test_set, runPath):
        ...
        def on_epoch_end(self, epoch, logs=None):
            if not test_set == None:
                ...
                for i in range(self.num_samples):
                    ...
                    # As updated DepthNorm does not modify the data, original image is used here
                    x_train, y_train = x_train[0], np.clip(DepthNorm(y_train[0], maxDepth=1000), minDepth, maxDepth)
                    x_test, y_test = x_test[0], np.clip(DepthNorm(y_test[0], maxDepth=1000), minDepth, maxDepth)
    ...
    # Callback: save checkpoints every 5 epochs
    callbacks.append(keras.callbacks.ModelCheckpoint(runPath + '/weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss',
        verbose=1, save_best_only=False, save_weights_only=False, mode='min', save_freq='epoch', period=5))
